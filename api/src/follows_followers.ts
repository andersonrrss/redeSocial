  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`/follows?username=${username}`, {
      ...
      .then((data) => {
        ...
        let html = "";
        console.log(data.follows_infos)
        // Itera sobre todos os seguidores para mostrá-los
        data.follows_infos.forEach((user : User) => {
          html += `
              <a href="/${user.nome}" class="flex justify-start items-center w-full p-3 h-[60px] hover:bg-gray-200">
                  <img src="../static/${user.profile_pic}" class="max-w-full rounded-full h-full mr-2"> 
