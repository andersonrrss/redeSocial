...
export function fullScreen(btn : any) {
...
export function checkTextLength(postContainer: any){
    const paragraphElement = postContainer.querySelector(".imagePostContent")
    const readMoreButton = postContainer.querySelector(`.readMoreBtn`)

    if(!paragraphElement || !readMoreButton){
        return
    }
    
    const lineHeight = parseInt(window.getComputedStyle(paragraphElement).lineHeight); // Obtém a altura da linha do texto
        ...
        readMoreButton.classList.remove('hidden'); // Remove a classe "hidden" para exibir o botão
        
        readMoreButton.addEventListener("click", function () { // Adiciona um evento de clique ao botão
            console.log("botão clicado")
            readMore(readMoreButton, paragraphElement)
        });
}

function readMore(button: any, postContent: any){
    ...
    if (isExpanded) { // Se o conteúdo estiver expandido
        ...
        button.textContent = 'Ler Mais'; // Altera o texto do botão para "Ler Mais"
        ...
        button.textContent = 'Ler menos'; // Altera o texto do botão para "Ver Menos"
    }
}   

export function likePost(button : any){
    const postId = (button as HTMLElement).getAttribute('post_id');
    try {
        fetch(`/likePost?post_id=${postId}`)
...
export function showComments(commentButton : any){
    console.log("função chamada")
    let postId = commentButton.getAttribute("post_id")
    alert("perdão, ainda vou adicionar os comentários, expresse suas ideias mentalmente...")
}