...
interface Message{
  ...
  sender_name: string
  timestamp: number
  responded_message_content: string
  responded_message_id: number
}
  ...
  if (send){
    ...
    message_element.innerHTML = `
      ...
      <div class="flex flex-col message sent">
        ${message.responded_message_id > 0 ? `
...
message_form.addEventListener("submit", function (event) {
  ...
  if (!!message && !!message.trim()) {
    ...
    message_input.value = "";
    fetch(
      `/sendmessage?message=${message.trim()}&receiver_id=${receiver_id}&chat_id=${chat_id}&responded_message_id=${reply_id}` // Informações necessárias para o envio da mensagem via servidor
    )
      .then((response) => {
        if (!response.ok) {
